...
type OptionalMetricKey = "heart_rate" | "altitude_m" | "cadence" | "power";
type MetricKey  = "speed" | OptionalMetricKey;
type SpeedMode  = "speed" | "pace";
...
function metricUnit(key: MetricKey, speedMode: SpeedMode): string {
  switch (key) {
...
function metricValue(p: TrackPoint, key: MetricKey, speedMode: SpeedMode): number | null {
  switch (key) {
    ...
    case "speed": {
      // Unlike an earlier version of this function, a real 0 (or near-0) is
      // NOT hidden here — decelerating to a stop is real, informative data
      // (confirmed against the reference activity's km 3.80: heart rate
      // stays plotted through a genuine ~20-sample taper to a full stop,
      // and hiding the matching speed made it look like a data gap/outlier
      // rather than what it actually is). Isolated sensor glitches are
      // handled by the outlier filter instead (displayTrack), which — unlike
      // a blanket "drop all zeros" rule — leaves a real gradual deceleration
      // alone since it isn't an isolated spike.
...
// Pace is minutes-with-a-fraction (e.g. 5.83) — always render it as m:ss,
// never a raw decimal. Note: unlike fmt.ts's fmtPace, this does NOT convert
// units itself — metricValue() above already returns the value in whatever
// unit is currently active, so converting again here would double-convert.
function fmtMetricValue(key: MetricKey, v: number, speedMode: SpeedMode): string {
...
// A handful of samples right before a real full stop are genuine, not
// noise (e.g. pace shooting up as speed trails toward 0), but they can still
// be extreme enough to be a poor basis for the *whole activity's* axis
// scale. Using the 95th-percentile deviation/range instead of the raw
// max means those samples still plot — Recharts simply clips a line past
// its axis domain — without a few real-but-extreme points squashing the
// other ~95% of normal values into a sliver of the chart height.
...
// Y-axis for each active metric is centered on that metric's own mean, not
// its raw min/max — lets differently-scaled series (bpm vs km/h vs watts)
// overlay on one chart without a dual/multi-axis "arbitrary scale alignment"
// lie: each axis is still real and derived only from its own data, just
// aligned at the mean so relative peaks/dips line up in time. Used for the
// overlay chart; standalone cards use axisDomainMinMax instead (see below).
function axisDomainCentered(points: TrackPoint[], key: MetricKey, speedMode: SpeedMode): [number, number] {
...
// Standalone single-metric cards show the metric's own real range (not
// mean-centered) — there's no other series to compare against, so the
// natural min/max (with a little padding) is more readable. Same
// percentile-based robustness as axisDomainCentered.
function axisDomainMinMax(points: TrackPoint[], key: MetricKey, speedMode: SpeedMode): [number, number] {
...
// ── Outlier detection ─────────────────────────────────────────────────────
// Delta-based, not statistical (a z-score/IQR approach is harder to reason
// about for this kind of data and doesn't behave well around legitimate
// stops/sprints). "Isolated spike" rule: a point is flagged only when it
// differs from BOTH its previous and next valid neighbor faster than a
// per-second rate a human can set directly (e.g. "speed can't plausibly
// jump 2 m/s in one second"). A genuine sustained change — a real sprint —
// only produces one big delta at the transition and then stays elevated, so
// it's never flagged; only a value that jumps away and immediately back is.
function sampleGapSec(a: TrackPoint, b: TrackPoint): number {
...
// A second, independent outlier rule for Speed/Pace specifically: an
// absolute floor, not a delta — any sample slower than "walking pace or
// slower" is dropped outright regardless of whether it looks like an
// isolated spike. This is deliberately a different kind of filter (a
...
// ── Pause detection ───────────────────────────────────────────────────────
// Two independent methods, picked per-activity (not blended) by whether real
// per-point wall-clock timestamps are available:
//
//  - timestamps (primary, used whenever every point has timestamp_unix —
//    i.e. Garmin activities parsed/reprocessed after this field was added):
//    this device stops recording points entirely during an auto-pause (it
//    doesn't keep sampling with a frozen clock), so a real pause shows up as
//    a plain gap between two CONSECUTIVE recorded points' timestamp_unix —
//    no speed/clock heuristics needed. (FIT's per-record elapsed_time field
//    was tried first as a "moving clock" signal here, but turned out to lag
//    wall-clock throughout the entire activity, not just during pauses —
//    e.g. one real run's last point read only 10:44 on that clock despite
//    50:35 of real wall-clock time — so it's not a trustworthy per-record
//    time source at all and duration must come from timestamp_unix.)
//  - heuristic (fallback, used when timestamp_unix isn't available — Strava
//    activities, or Garmin ones from before this field existed): a sustained
//    run of near-zero speed. Debounced — a single blip of non-slow speed
//    inside an otherwise-slow stretch does NOT end the run (only a
//    *sustained* faster stretch does) — without this, one noisy GPS/speed
//    reading mid-pause fragments a real 55-minute stop into several much
//    shorter ones.
interface Pause { afterIndex: number; durationSec: number; }
const PAUSE_SPEED_EPS = 0.3; // m/s
const PAUSE_RESUME_TOLERANCE_SEC = 10;

function detectPausesFromTimestamps(points: TrackPoint[], thresholdSec: number): Pause[] {
  const pauses: Pause[] = [];
...
function detectPausesHeuristic(points: TrackPoint[], thresholdSec: number): Pause[] {
  const pauses: Pause[] = [];
...
function detectPauses(points: TrackPoint[], thresholdSec: number): Pause[] {
...
function fmtPauseDuration(sec: number): string {
...
function fmtElapsedClock(sec: number): string {
  const h = Math.floor(sec / 3600), m = Math.floor((sec % 3600) / 60), s = Math.floor(sec % 60);
  const mm = String(m).padStart(2, "0"), ss = String(s).padStart(2, "0");
  return h > 0 ? `${h}:${mm}:${ss}` : `${m}:${ss}`;
}

// Pale → deep yellow, purely by magnitude — a single at-a-glance "how much"
// signal, deliberately NOT a proportional-width gap (every flag renders at
// the same small fixed size; only this color carries the magnitude
// information). Yellow specifically (not the earlier pink→red for pauses)
// so the flag's black foreground text stays readable across the whole
// gradient — yellow hues stay high-lightness even at full saturation, unlike
// red which gets dark enough at the high end to fight with black text.
// Shared by pause-duration flags and HR-recovery flags (same visual language
// for "how big was this," just two different underlying units/caps).
function magnitudeColor(magnitude: number, cap: number): string {
...
// ── Chart data: a synthetic "x" axis where every pause collapses to the
// same small fixed notch, regardless of its real duration or the X mode
// (distance/time) — duration is only ever conveyed by the flag's color, not
// by how much horizontal space the pause takes up.
interface ChartRow { x: number; realX: number | null; pauseDurationSec?: number; pauseAfterIndex?: number; [key: string]: number | string | null | undefined; }

function buildChartData(
  points: TrackPoint[], pauses: Pause[], xMode: XMode, metrics: MetricKey[], speedMode: SpeedMode,
  // Points where outlier filtering nulled the value — see the "Remove
  // outliers" checkbox. Consistency: an excluded point doesn't just lose its
  // Line value, it also stops "spending" x-axis width — its step
  // contributes ZERO instead of its real distance/time span. This is
  // deliberately NOT the same "collapse to a small notch" treatment pauses
  // get: a pause's real span is always huge relative to the notch, so
  // shrinking it down reads as "compressed." An outlier stretch is usually
  // already a small span (e.g. a runner decelerating to a stop covers very
  // little real distance) — giving it a pause-sized notch would actually
  // *inflate* its width past its real size, the opposite of "cut."
  outlierMask: boolean[] = [],
): ChartRow[] {
  // Time mode uses real wall-clock elapsed time (timestamp_unix minus the
  // activity's first timestamp) whenever available, NOT elapsed_sec — that
  // FIT field isn't a trustworthy per-record time source (see the pause
  // detection notes above). Falls back to elapsed_sec only when
  // timestamp_unix is missing (Strava, or pre-backfill Garmin rows), where
  // it's the best available approximation.
...
function xTickFormatter(rows: ChartRow[], xMode: XMode) {
...
// ── Heart-rate recovery flags ─────────────────────────────────────────────
// One flag per detected pause, shown only on the Heart rate card: the drop
// (or rise) in HR from just before stopping to right after resuming — a
// different signal from the generic pause-duration flag, so it gets its own
// shape/color rather than reusing PauseFlagShape.
interface HrRecoveryFlag { afterIndex: number; delta: number; }

function nearestHr(points: TrackPoint[], startIdx: number, dir: 1 | -1): number | null {
...
function computeHrRecovery(points: TrackPoint[], pauses: Pause[]): HrRecoveryFlag[] {
