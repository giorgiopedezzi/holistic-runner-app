...
export function createDateRangesController(ctx: AppContext) {
  ...
  async function validate(req: import("http").IncomingMessage, body: Body, excludeId: number | null): Promise<{ name: string; activityId: number | null }> {
    ...
    const existing = await ownedRanges(req).byName(name);
    ...
    if (body.activity_id != null) {
      ...
      const activity = await ownedActivities(req).byId(activityId) as unknown as
  ...
  const create: Handler = async (req, res) => {
    ...
    const { name, activityId } = await validate(req, body, null);
    const row = await ownedRanges(req).create(name, body.from!, body.to!, activityId);
    if (!row) throw new Error("Date range insert did not return a row.");
  ...
  const update: Handler = async (req, res, url) => {
    ...
    if (!await ownedRanges(req).byId(id)) throw notFound(`No date range with id ${id}.`);
    const body = await readJsonBody<Body>(req);
    const { name, activityId } = await validate(req, body, id);
    return send(res, await ownedRanges(req).update(id, name, body.from!, body.to!, activityId));
  };
  ...
  const remove: Handler = async (req, res, url) => {
    ...
    if (!Number.isInteger(id)) throw badRequest("Invalid date range id.");
    if (!await ownedRanges(req).byId(id)) throw notFound(`No date range with id ${id}.`);
    await ownedRanges(req).remove(id);
