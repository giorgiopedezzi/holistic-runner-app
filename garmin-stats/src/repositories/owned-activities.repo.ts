...
export function createOwnedActivitiesRepo(db: Queryable, userId: string) {
  return {
    ...
    summary: (from: string, to: string) => db.all("SELECT sport,COUNT(*)::int AS total_activities,ROUND((SUM(distance_m)/1000)::numeric,2) AS total_km,ROUND((SUM(duration_sec)/3600)::numeric,2) AS total_hours,SUM(calories)::int AS total_calories,ROUND(AVG(avg_hr))::int AS avg_hr,ROUND(AVG(avg_pace_minkm)::numeric,2) AS avg_pace,ROUND(SUM(ascent_m)) AS total_ascent FROM activities WHERE user_id=$1 AND date_only BETWEEN $2 AND $3 AND sport IS NOT NULL AND deleted_at IS NULL GROUP BY sport ORDER BY total_km DESC", [userId, from, to]),
    weekly: (from: string, to: string) => db.all("SELECT to_char(to_date(date_only, 'YYYY-MM-DD'), 'IYYY-\"W\"IW') AS week,COUNT(*)::int AS runs,ROUND((SUM(distance_m)/1000)::numeric,2) AS km,ROUND(AVG(avg_hr))::int AS avg_hr,ROUND(AVG(avg_pace_minkm)::numeric,2) AS avg_pace FROM activities WHERE user_id=$1 AND date_only BETWEEN $2 AND $3 AND deleted_at IS NULL GROUP BY week ORDER BY week", [userId, from, to]),
    monthly: (from: string, to: string) => db.all("SELECT substring(date_only, 1, 7) AS month,COUNT(*)::int AS runs,ROUND((SUM(distance_m)/1000)::numeric,2) AS km,ROUND(AVG(avg_hr))::int AS avg_hr,ROUND(AVG(avg_pace_minkm)::numeric,2) AS avg_pace,ROUND(SUM(ascent_m)) AS ascent FROM activities WHERE user_id=$1 AND date_only BETWEEN $2 AND $3 AND deleted_at IS NULL GROUP BY month ORDER BY month", [userId, from, to]),
