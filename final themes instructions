...
The running graph must remain the **visual centerpiece of the dashboard**.

The main content hierarchy should be:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ↓
PRIMARY METRICS
Activity / Distance / Time / Calories
    ↓
SECONDARY METRICS
Pace / Average Distance / Elevation / HR
```

---

# 13. Main Graph Header

The area above the main graph must be compact and structured.
...
* `Running – Andamento distanza e ritmo`
...
Keep these elements on the same baseline.

### Row 2

Left:

Pace/activity legend.

Right:

Two compact KPI cards:
...
Immediately after this row, start the graph.

Do NOT insert additional cards, filters or whitespace between the KPI/legend row and the chart.

---

# 14. KPI Cards

The distance and pace KPIs above the graph should remain compact.

They are supporting information, not the primary visual focus.

Do not repeat the same KPIs immediately below the graph.

Do not place Activity, Time, Calories, Elevation or Heart Rate above the main graph.

The graph itself should occupy approximately **70–80% of the main card's visual area**.

---

# 15. Icon System

Introduce icons consistently across the dashboard.
...
```text
Running section     → running/shoe icon
Date selector       → calendar
Dropdown             → chevron-down
Activity             → running/shoe
...
Icons should clarify meaning, not become decoration.

---

# 16. Primary Metric Cards

Below the comparison graph, place the four primary metrics in one horizontal row:
...
* comparison indicator where relevant

---

# 17. Secondary Metrics

Below the primary metrics:
...
These metrics should have less visual weight than the main graph.
...
* Keep the main graph first.
* Keep the graph header compact.
* Allow the KPI cards to stack when necessary.
* Keep the chart readable.
* Convert the four primary metrics into a responsive grid.
* Stack secondary metrics when required.
* Never allow horizontal overflow.

The visual hierarchy must remain:

**Main graph → comparison → primary metrics → secondary metrics**
