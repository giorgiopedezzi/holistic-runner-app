...
- **Automatic ambient glow is the DEFAULT, not the only option — a background picker is back in SettingsTab.tsx (BackgroundPicker), reversing the 2026-08-16/08-17 correction pass per later explicit product feedback.** `index.css`'s `body::before` still paints the same page-sized single-hue radial ramp as its fallback value — `background: var(--bg-image, radial-gradient(135% 135% at 0% 0%, color-mix(accent var(--ambient-accent-mix), --bg) 0%, --bg 46%, color-mix(black var(--ambient-dark-mix), --bg) 100%))`, `background-size: cover`, `background-position: center` — lighter at the top-left, darker toward the bottom-right, unchanged from the corrected render. `useAppearance.ts`'s `applyBackground()` (called from `applyToDocument`) now sets `--bg-image` on `:root` again: a bundled preset (`utils/backgrounds.ts`'s `BUNDLED_BACKGROUNDS`, still 4 CSS-gradient presets — Aurora/Sunset/Ocean/Midnight) writes its own `css` value directly; a custom upload wraps `api.settings.backgroundImageUrl(value)` in `url(...)`; `"none"` clears the property, which is what lets the ambient-gradient fallback show through again. A real bundled/custom background also sets `data-background="bundled"|"custom"` on `:root`, which disables the shimmer keyframe (a fixed picture, not a color-cycling wash) via `:root[data-background="bundled"] body::before, :root[data-background="custom"] body::before { animation: none; }` — same mechanism Graphite's own `animation: none` override already used. `types/api.ts`'s `background_kind`/`background_value` fields and their backend routes (`PUT .../background`, `POST .../background/upload`, `GET .../background-image`) were never removed even during the correction pass — only the frontend UI/JS was disconnected — so this reversal is wiring, not new backend work.
