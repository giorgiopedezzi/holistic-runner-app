...
const FEATURE_INTERESTS = ["multi_user_coach", "shared_groups"] as const;
...
export function createFeedbackController(ctx: AppContext) {
  ...
  const create: Handler = async (req, res) => {
    ...
    if (!hasFreeText && !hasPricing && !hasPricingWhyNot && !hasFeatureInterest && !hasFeatureOther) {
      throw unprocessable("At least one of free_text, pricing_choice, pricing_why_not_free_text, feature_interest, or feature_interest_other_free_text must be provided.");
    }
