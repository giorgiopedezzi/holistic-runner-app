...
| `/api/v1/plan-instances/:id` | A resolved instance of a template — the instantiate response's own `Location` target — plus its `days` (all `plan_instance_days` rows, each with `segments` as JSON), `approved_at` (HRA-113), `name`/`event` (HRA-114), and `race_name`/`race_date` (HRA-121, both nullable) |
