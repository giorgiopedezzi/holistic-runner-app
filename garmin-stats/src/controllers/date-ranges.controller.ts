...
export function createDateRangesController(ctx: AppContext) {
  ...
  const list: Handler = async (_req, res, url) => {
    ...
    const total = (await repo.count())?.count ?? 0;
    return send(res, paginated(await repo.listPage(limit, offset), total, limit, offset));
  };
  ...
  async function validate(body: Body, excludeId: number | null): Promise<{ name: string; activityId: number | null }> {
    ...
    const existing = await repo.byName(name);
    ...
    if (body.activity_id != null) {
      ...
      const activity = await activitiesRepo.byId(activityId) as unknown as
  ...
  const create: Handler = async (req, res) => {
    ...
    const { name, activityId } = await validate(body, null);
    const row = await repo.create(name, body.from!, body.to!, activityId);
  ...
  const update: Handler = async (req, res, url) => {
    ...
    if (!await repo.byId(id)) throw notFound(`No date range with id ${id}.`);
    const body = await readJsonBody<Body>(req);
    const { name, activityId } = await validate(body, id);
    return send(res, await repo.update(id, name, body.from!, body.to!, activityId));
  };
