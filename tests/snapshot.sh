...
# no-param endpoints
fetch range                 "/api/v1/range"
fetch body_range            "/api/v1/body-measurements/range"
fetch settings              "/api/v1/settings"
fetch activities_trash      "/api/v1/activities/trash"
fetch body_trash            "/api/v1/body-measurements/trash"

# range endpoints @ FULL window
fetch activities_full       "/api/v1/activities?${FULL}"
fetch activities_count_full "/api/v1/activities/count?${FULL}"
fetch summary_full          "/api/v1/summary?${FULL}"
fetch weekly_full           "/api/v1/weekly?${FULL}"
fetch monthly_full          "/api/v1/monthly?${FULL}"
fetch body_list_full        "/api/v1/body-measurements?${FULL}"
fetch body_count_full       "/api/v1/body-measurements/count?${FULL}"
fetch body_monthly_full     "/api/v1/body-measurements/monthly?${FULL}"
fetch body_correlation_full "/api/v1/body-measurements/correlation?${FULL}"

# range endpoints @ NARROW window (activity data present, body empty here — both are signal)
fetch activities_narrow     "/api/v1/activities?${NARROW}"
fetch summary_narrow        "/api/v1/summary?${NARROW}"
fetch weekly_narrow         "/api/v1/weekly?${NARROW}"
fetch monthly_narrow        "/api/v1/monthly?${NARROW}"
fetch body_list_narrow      "/api/v1/body-measurements?${NARROW}"
fetch body_correlation_narrow "/api/v1/body-measurements/correlation?${NARROW}"

# id endpoints
fetch activity_200          "/api/v1/activities/${ACT_ID}"
fetch activity_1            "/api/v1/activities/${ACT_ID2}"
fetch track_200            "/api/v1/activities/${TRACK_ID}/track"
