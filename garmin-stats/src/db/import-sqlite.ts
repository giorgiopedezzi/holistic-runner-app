...
async function main(): Promise<void> {
  ...
  try {
    ...
    await client.query(`UPDATE activities SET
      system_classification = CASE
        ...
        ELSE COALESCE(statistical_classification, ai_classification, CASE WHEN user_feedback = 'approved' THEN final_classification END)
      END,
      system_explanation = CASE
        ...
        ELSE NULL
      END,
      manual_classification = CASE WHEN user_feedback = 'rejected' THEN final_classification ELSE NULL END`);
