...
- **Pause detection is two independent methods, picked per-activity** by whether every `track_points` row has `timestamp_unix`: `detectPausesFromTimestamps` (primary, real Garmin data) is a plain gap ≥ threshold between consecutive points' `timestamp_unix` — this device stops recording entirely during an auto-pause, so no speed/clock heuristics are needed, and `elapsed_sec` isn't used at all (see "FIT parser notes" for why it's unreliable). `detectPausesHeuristic` (fallback, Strava or not-yet-backfilled Garmin) uses a debounced near-zero-speed run — a single noisy speed blip inside an otherwise-slow stretch does not end the run, or one real long stop fragments into several short ones. **HRA-337:** each detected `Pause` now also carries `recorded` (`true` for the timestamp path, `false` for the heuristic) — surfaced through `PauseInspectionRow.recorded` and the "Pauses (N)" dialog's Source column as "Recorded"/"Inferred" provenance, and (independently) mirrored server-side by `garmin-stats/src/domain/workout-metrics.ts`'s `detectPauseEvents` for the single-workout report's own pause evidence — the same timestamp/heuristic split, not a second algorithm, since the backend needs it over `track_points` it already loaded for HR/stamina rather than round-tripping through this frontend module.
...
- **Automatic ambient glow is the DEFAULT, not the only option — an upload-only background picker is back in SettingsTab.tsx (BackgroundPicker), reversing the 2026-08-16/08-17 correction pass per later explicit product feedback.** `index.css`'s `body::before` still paints the same page-sized single-hue radial ramp as its fallback value — `background: var(--bg-image, radial-gradient(135% 135% at 0% 0%, color-mix(accent var(--ambient-accent-mix), --bg) 0%, --bg 46%, color-mix(black var(--ambient-dark-mix), --bg) 100%))`, `background-size: cover`, `background-position: center` — lighter at the top-left, darker toward the bottom-right, unchanged from the corrected render. `useAppearance.ts`'s `applyBackground()` (called from `applyToDocument`) sets `--bg-image` on `:root`: a custom upload wraps `api.settings.backgroundImageUrl(value)` in `url(...)`; "Remove background" (kind `"none"`) clears the property, which is what lets the ambient-gradient fallback show through again. A real custom background also sets `data-background="custom"` on `:root`, which disables the shimmer keyframe (a fixed picture, not a color-cycling wash) via `:root[data-background="custom"] body::before { animation: none; }` — same mechanism Graphite's own `animation: none` override already used. `BackgroundPicker` itself has no bundled-preset UI (dropped per a follow-up feedback round — upload/remove only), but `background_kind`'s `"bundled"` value and `utils/backgrounds.ts`'s `BUNDLED_BACKGROUNDS` presets are untouched on the type/backend/`applyBackground` side, so a row already set that way (from before this simplification, or a future direct API write) still renders its preset gradient correctly — there's just no picker path left to choose one. `types/api.ts`'s `background_kind`/`background_value` fields and their backend routes (`PUT .../background`, `POST .../background/upload`, `GET .../background-image`) were never removed even during the correction pass — only the frontend UI/JS was disconnected — so this reversal is wiring, not new backend work.
...
- **Plan templates/instances moved out (HRA-139)** — see the new "Plans tab" section below; this tab no longer renders either card.

## Plans tab (PlansTab.tsx, tab label "Training plans")
- **Moved out of Manage tab into its own top-level "Training plans" tab (HRA-139).** `PlanTemplatesSection`/`PlanInstancesSection` (and the `templates` lift-and-share state) now live in `PlansTab.tsx`, not `ManageTab.tsx` — pure relocation, no behavior change to either section. Everything below this bullet predates the move and still describes the two sections themselves accurately; only their container changed.
- **`PlanTemplatesSection`** (`components/manage/PlanTemplatesSection.tsx`, HRA-117): the training-plan template CRUD card, rendered first on the Plans tab (HRA-139), above `PlanInstancesSection`. Two modes, `list`/`editor` (own local state, not a route):
  ...
  - **Accordion-based inline editing, Restore, pending-changes warning (HRA-140).** The old `mode: "list" | "editor"` full-screen swap is gone — every list row is its own `AccordionCard` (`ui/AccordionCard.tsx`, the same single-expand pattern `SettingsTab` already uses) that expands in place to reveal exactly the same editor fields the old `mode === "editor"` screen showed (AC1) — the row header itself is now the click target (no separate "Edit" button; a header can't nest a real `<button>`, mirroring `ActivityRow.tsx`'s own established constraint). Only ONE row's edits ever live in the "live" editor state at a time (`name`/`event`/`distanceValue`/`editor`/`planWarnings`, unchanged from before) — same single-active-editor architecture, just keyed by a new `activeKey: number | "new" | null` instead of the page-level `mode`; `editingId` is now a plain derived const (`typeof activeKey === "number" ? activeKey : null`), not its own state. **Collapsing a dirty row, or opening a different one while the current row is dirty, never discards anything** — `stashCurrentIfDirty()` snapshots the live fields into a `drafts: Record<string, Draft>` map (keyed by template id or `"new"`) whenever `isEditorDirty()` (a new helper diffing the live fields against new `baselineName`/`baselineEvent`/`baselineDistanceValue` state, plus the pre-existing `savedDslSource` for the DSL text) is true; reopening that same row later restores the draft (`reopenDraft()`, re-running `runGenerate` against the stashed `dslSource` to rebuild `editor.sections` rather than also stashing the resolved tree, which could drift from it) — a collapsed row with an entry in `drafts` gets the warning icon (Ask #4); one without gets the plain "Open to edit" hint (Ask #2) — both rendered via a shared `rowStatusHint()`, italic `text-secondary` for the hint, a `lucide-react` `AlertTriangle` styled via the pre-existing `.hra-text-warning` class (not an inline color, per this repo's theming rule) with a native `title` tooltip for the icon. **"Cancel" is renamed "Restore"** (new `common.restore` i18n key, since HRA-141 will need the identical label for the instance card) — gated on a confirm modal only when `isEditorDirty()`, using the same pending-then-confirm/cancel modal shape `PlanInstancesSection` already established; a clean row restores (resets local state + collapses) immediately, no modal. Restoring never needs to "re-populate" fields from the server — the list row itself only ever renders the real persisted `templates` data, never mutated by local typing, so resetting local state and collapsing already IS "reverting to the last-saved values." A stashed draft's `drafts` entry is removed only by a successful Save or an explicit Restore, never by simply reopening the row (AC4's own wording). **"New template" creation (AC5)** is a `"new"`-keyed row rendered above the list whenever `activeKey === "new"` or a `"new"` draft exists (`newDraftPending`) — the "+ New template" button disables itself while that's true, since only one concurrent unsaved-new draft is supported. **The "How to use it" help button/modal, previously duplicated between the list-mode and editor-mode headers (identical content either way), consolidates to one top-level instance** — a minor simplification that fell out of removing the mode swap, not a new feature. `canSave`/`canApprove`/the zero-warning rule (AC6) are byte-identical formulas to before, just reading the same variables under their new derivation. **Review fix, round 1 (superseded by round 2 below): Delete moved inside the accordion's own panel**, gated on the row being expanded. **Review fix, round 2 (further explicit feedback — an overlay, not a panel-only control): Delete is a `Trash2` icon button positioned with `position: absolute` + `zIndex: 1` on top of the collapsed `AccordionCard`'s own header**, just left of its chevron, wrapped in a `position: relative` sibling `<div>` — a real DOM SIBLING of the `AccordionCard`, never nested inside its header `<button>` (still invalid HTML either way), just visually overlaid so it reads as "inside the accordion" rather than a separate column beside it. Because the two elements don't overlap in the DOM, only on screen, the icon intercepts its own click independent of the header's `onToggle` with no `stopPropagation` needed — and as a bonus over round 1, Delete now works whether the row is expanded or collapsed, same reach it had before this Story touched the card at all. One shared confirm modal (not per-row) renders at the `Card` level, keyed off `deleteConfirmId` alone — no `activeKey`/`editingId` coupling needed any more, since the modal (a full-screen backdrop, z-index 100) blocks all other interaction while pending, so the round-1 "reset `deleteConfirmId` on every toggle" workaround was removed as no longer solving a real problem. **Same pattern to carry into HRA-141** for the instance card's own delete button.
- **`PlanInstancesSection`** (`components/manage/PlanInstancesSection.tsx`, HRA-118): the plan-instance CRUD card, rendered just below the template card. Per-row `AccordionCard`s, keyed by `activeKey: number | "new" | null` (HRA-133 first unified the separate `instantiate`/`editor` modes into one shared-field screen; HRA-141 later replaced the `mode: "list" | "plan"` full-screen swap that screen used with the accordion — see both Stories' own bullets below). Structural sibling of `PlanTemplatesSection` but **simpler at save time**: each day `PUT`s its own `{section_name, week_number, date, dsl}` directly (HRA-115) — there's no whole-document `dsl_source` to content-anchor-patch here, only a local `SectionView[]` mirror updated in place per edit.
  - **List** — `api.planInstances.list()` (added this Story, `GET /api/v1/plan-instances`, optionally `?template_id=`; no prior endpoint returned more than one instance at a time — confirmed with the user before adding it, since it's backend work inside a Story framed as frontend-only). One row per instance: `name`, `event`, `start_date`, an approved/not-approved `Badge`. (HRA-141 later moved Edit/Delete into the accordion pattern — see that Story's own bullet below; this bullet describes the original HRA-118 shape.)
  ...
  - **Unified instantiate/editor screen (HRA-133).** The `instantiate` and `editor` modes — previously two structurally separate `<Card>` render trees under `if (mode === "instantiate")`/the trailing unconditional return — are merged into one `mode: "list" | "plan"` render. `instName` is now the ONE name field for both creating fresh and displaying an existing instance (the separate `editName` state this file used to carry is gone). Two new derived flags gate the shared top fields (template picker, race name/date/url, start date, days-before-race, rest-day label, race-pace-anchor/pace-mode toggle, the whole anchor table): `fieldsLocked = editingId != null` (an instance exists — freshly created this session or loaded) and `fieldsDisabled = fieldsLocked || !formEnabled`. **Name is the one exception** (`nameDisabled = fieldsLocked ? isApproved : !formEnabled`) — it was already editable in the pre-unification editor (governed only by `isApproved`), so it keeps that exact rule rather than falling under the new lock, per this Story's own "no regression to day-level editing" AC. `startEdit()` now populates every shared field from the loaded instance's real persisted values (`templateId`, `startDate`, `raceName`/`raceDate`/`raceUrl`, and the anchor table via a new `paceValueToAnchorRow()` — the reverse of `parsePaceOverrideInput`, mapping a stored `PaceValue` back to the table row's absolute/relative display fields) — this is what makes AC1 ("same screen shape whether fresh or existing") literally true rather than a layout coincidence, since the exact same JSX renders in both cases, just disabled once `fieldsLocked`. **Known, unavoidable display limitation**: `goal_time`/`race_pace_anchor` are collapsed into a plain absolute `PaceValue` at instantiate time server-side (`garmin-stats` controller) — there is no persisted way to tell "this came from goal time" apart from a raw absolute override, so a loaded instance's anchor table always shows every override as an Anchor-override-style row, never reconstructing Goal time mode. `week1AnchorMismatch` (the live-form check) and `editorWeek1AnchorMismatch` (the loaded-instance check, HRA-130) are now both evaluated on the same unified screen — either firing shows the warning; kept both rather than removing the now-partially-redundant `editorWeek1AnchorMismatch`, since determining it's truly safe to delete is a separate judgment call outside this Story's restructuring-only scope (candidate, not applied). `onInstantiate`'s success path no longer resets the top fields afterward (they already hold exactly what was submitted, and now stay visible as the instance's own values) or changes `mode` (already `"plan"` when Create is clicked). `ui/Select.tsx` gained a `disabled?: boolean` prop (forwarded to Radix's own `SelectPrimitive.Root`) — needed so the template picker can actually lock once `fieldsLocked`, not just look inert; no existing caller was affected (optional, backward-compatible).
  - **Edit start date + pace anchors post-creation, with a cutover (HRA-134).** Two of HRA-133's locked-but-populated top fields — `startDate` and the anchor table's own rows — become genuinely editable once an instance exists and isn't approved yet, via a new `editableFieldDisabled = fieldsLocked ? isApproved : !formEnabled` (the same formula `nameDisabled` already used, just extended to these two). The template picker, race name/date/url, rest-day-label, and the race-pace-anchor/Goal-time sub-flow stay on the original `fieldsDisabled` — deliberately out of scope (AC1 only names start_date and pace anchors; Goal-time specifically can't be meaningfully unlocked anyway, since HRA-133 already established that a loaded instance can never reconstruct which override came from Goal time). Editing either field surfaces a "Modification start from" cutover picker + Regenerate button (`pendingRegenerateChange` — `startDate`/`anchorRows` diffed against `baselineStartDate`/`baselineAnchorRows`, two new snapshots taken at every successful load/create/save/regenerate, not against the raw persisted `pace_overrides` JSON, which is a resolved `PaceValue` shape rather than the UI's own absolute/relative row shape). `effectiveFrom` defaults to today and is floored there by the `DatePicker`'s own `min` prop client-side — the real floor is server-side (HRA-132), never trusted from the client alone. Clicking Regenerate (`onRegenerateClick`) first counts manually-edited days on/after the cutover via `manualEditCount()` — a new `persistedDsl` snapshot (`date -> dsl`, refreshed at the same four points as the baseline pair) diffed against live `sections`, catching a day touched by direct `dsl` edit, a day swap, or a week swap alike — and only asks for confirmation (same pending-then-confirm/cancel shape `pendingTemplateId`/`pendingDaySwap` already established) when that count is `> 0`; zero manual edits in range regenerates immediately. `api.planInstances.regenerate()` (new client method, `POST /api/v1/plan-instances/:id/regenerate`) always sends a **complete** `pace_overrides` map, even `{}` — unlike `onInstantiate`'s own override-building, which omits the field when empty, sending `{}` here is deliberate: omitting it server-side means "keep the instance's current stored overrides," which would silently ignore the user having cleared every anchor row. A successful regenerate re-baselines `startDate`/`anchorRows`/`persistedDsl` to the new current values, so the cutover UI disappears until something changes again.
  - **Full field editability, rename warning, Save/Regenerate state machine (HRA-136).** Every shared top field but the Template picker is now editable once an instance exists and isn't approved — the three earlier Stories' field-by-field exceptions (`fieldsDisabled`/`nameDisabled`/`editableFieldDisabled`) collapse into one `fieldDisabled = fieldsLocked ? isApproved : !formEnabled`, applied everywhere except the Template `Select` (still gated on bare `fieldsLocked`). **Two disjoint dirty buckets** replace the old single `pendingRegenerateChange`: `saveBucketDirty` (Name/Race name/Race date/Race URL vs. new `baselineInstName`/`baselineRaceName`/`baselineRaceDate`/`baselineRaceUrl`, plus a new `anyDayDirty` — any day whose live `dsl` differs from `persistedDsl`, unscoped by cutover unlike HRA-134's own `manualEditCount`) and `regenerateBucketDirty` (`startDate`/`anchorRows` as before, now joined by `racePaceAnchor`/`paceMode` vs. their own new baselines, plus the goal-time H/M/S/distance-override fields when `paceMode === "goalTime"`, via `goalTimeFieldsChanged`). **Regenerate-dirty always wins**: `saveEnabled = !regenerateBucketDirty && (saveBucketDirty || saveForcedEnabled)` — editing a Regenerate-bucket field disables Save outright regardless of Save-bucket state, per the Story's own explicit confirmation. `saveForcedEnabled` implements AC5 ("a successful Regenerate re-enables Save unconditionally") — set `true` in `doRegenerate`'s success path, cleared by a real `onSave` (back to pure dirty-tracking) or by starting fresh; it still loses to a fresh `regenerateBucketDirty` the same as any other Save-bucket dirtiness would. **The goal-time/anchor-override toggle and the race-pace-anchor selection are now reachable post-creation** (previously locked, since HRA-133's own "loaded instance can never reconstruct Goal time" display limitation made unlocking them seem pointless) — `buildPaceOverridesForRegenerate()` now derives the race-pace-anchor's override from the same client-side `derivedPaceSecPerKm` the anchor table's preview already computes (formatted via `formatPaceSecPerKm`) rather than sending raw `goal_time`/`race_pace_anchor` fields, since the regenerate endpoint's own contract has no such fields and adding them is Epic HRA-36 territory, out of this Story's slice. **`onSave` now also PATCHes Race name/date/url** (HRA-135's endpoint accepts them alongside `name`/`days`) — sent unconditionally rather than diffed against baselines, since PATCH's own semantics make resending an unchanged value harmless. **Rename confirm (Ask #2)**: gated on the Save click, not per-keystroke — a modal per character typed would be unusable — via `onSaveClick`/`pendingNameChangeConfirm`, same pending-then-confirm/cancel shape every other consequential action in this file already uses; wording "This will rename the current plan — it won't create a copy. Continue?" per the Story's own draft text. **One action row (AC6)**: the earlier separate block that appeared/disappeared above the Save/Approve/Cancel row as `pendingRegenerateChange` toggled (moving everything below it — exactly the "moving UI" pattern CLAUDE.md otherwise forbids) is gone; the Regenerate unit (a small "Regenerate from" label + `effectiveFrom` `DatePicker` + button) is now a fixed sub-group inside that same row, always rendered once `fieldsLocked`, only ever *disabled* (not unmounted) until `regenerateBucketDirty` — same "visible but inert" treatment Approve already had for `isApproved`. **Review fix, round 1: the unit's own wrapper must not use `.hra-row-wrap`** — that class's `flex-wrap: wrap` (shared by the outer Save/Approve/Regenerate-unit/Cancel row too) let the label drop onto its own line above the date picker the instant the row ran out of horizontal width, splitting the "one unit" the Story explicitly asked for. Fixed with a plain inline `flexWrap: "nowrap"` + `flexShrink: 0` on the unit's own wrapper (and `whiteSpace: "nowrap"` on the label). **Review fix, round 2 (further explicit feedback): the three elements became one real control** — a button-shaped `<div>` (`.hra-btn.hra-regenerate-unit`, `data-variant="green"`) reading "Regenerate from" with the `DatePicker` nested INSIDE it, compact and borderless (`index.css`'s `.hra-regenerate-unit .hra-date-trigger` override — `height: auto; padding: 0; border: none; background: transparent; color: inherit`, shorter than the form's usual `--hra-field-h` since it now shares the outer button's own padding instead of sitting in its own row). A literal `<button>` can't contain the date picker's own nested `<button>` (invalid HTML, unpredictable click bubbling), so this follows `ActivityRow.tsx`'s own established card-as-button pattern instead: `role="button"` + `tabIndex` + `onKeyDown` on the div restore the same click/keyboard-activate behavior a real button gives for free, and a new `regenerateDisabled` const (the same three conditions the old plain `<button>`'s `disabled` attribute checked) drives both the click guard and a new `data-disabled` attribute — `index.css` gained a `.hra-btn[data-disabled]` rule mirroring `.hra-btn:disabled` exactly, since a `<div>` has no real `disabled` to hook. The nested `DatePicker`'s own click is wrapped in a `<span onClick={e => e.stopPropagation()}>` so opening the calendar doesn't also fire Regenerate — clicking anywhere else in the button (the label) does. **Known gap, flagged**: `restDayLabel` is now editable per AC1's literal "every field but Template," but neither the PATCH nor the regenerate endpoint accepts it — there is no persistence path for an edit to it once an instance exists, so it silently has no lasting effect post-creation; the Story's own two dirty-bucket definitions don't name it either, consistent with leaving it out of both. Candidate for Epic HRA-36 (add it to one of the two write endpoints) or a future Story disabling it once `fieldsLocked`, whichever the human prefers.
  - **Goal-time UX: mask, placement, race-pace consistency (HRA-137).** Goal time moves from its own separate row (appearing/disappearing with `paceMode`, another "moving UI" instance HRA-136's Regenerate-unit fix already addressed elsewhere in this same file) into Row 3, as a third `Field` alongside Race pace anchor and Pace input — always mounted now, merely `disabled={fieldDisabled || paceMode !== "goalTime"}` outside goalTime mode. **One masked `HH:MM:SS` text input replaces the old three separate H/M/S number fields** (`.hra-goal-time-fields`/`.hra-goal-time-unit`, both removed from `index.css`) — small custom mask per the Story's own explicit "zero-dependency, not a new library" instruction, not a generic `ui/` primitive since it's this field's only call site. The state shape changed to match: `goalH`/`goalM`/`goalS` (still read everywhere unchanged — `goalTimeToSec`, `pad2`, the instantiate body builder, `hasEnteredData`) are now *derived* 2-char slices of one `goalTimeDigits` raw-digit-buffer state (`""` = untouched, 0-6 digit chars, no colons — `formatGoalTimeDigits` inserts them for display, `sanitizeGoalTimeInput` strips everything non-digit from the field's own onChange value, which is what makes backspace work for free: a colon removed from the displayed text just gets stripped back out, net effect is the last real digit is gone). HRA-136's own goal-time dirty-tracking baselines (`baselineGoalH`/`M`/`S`) collapsed the same way, into one `baselineGoalTimeDigits`. **Bidirectional consistency (Ask #3)**: the same Goal-time field now does double duty — in `goalTime` mode it's the live editable source of truth; in `anchor` mode it becomes a **read-only preview of the equivalent Goal Time**, computed from the race-pace anchor's own Absolute-pace table row (`equivalentGoalTimeSec`, mirroring `derivedPaceSecPerKm`'s existing reverse-direction computation but only for an Absolute row value — a Relative-to-another-anchor row has no standalone pace to convert without resolving the whole policy chain, same scope boundary `derivedPaceSecPerKm` already respects) — tagged `"(from {{anchor}}'s pace)"`, next to the input, same placement style the anchor table's own `"(from goal time)"` tag already uses. `previewGoalDistanceM()`'s distance precedence (explicit override > template's own `distance_m` > event's standard distance) and the actual goal-time→pace conversion math are both completely untouched — this Story only changed the input's presentation layer and added the one new reverse-direction computation, so AC4 ("no regression") holds by construction, not by re-verification of logic that was never edited.
  - **Anchor table shows the resolved pace for Relative rows too (HRA-138).** A "Relative to" row (`row.relativeTo` set) used to show a blank, disabled Absolute input even once that anchor's Status column read "Resolved" — the resolved value existed (`resolution`, computed further up via `resolveIntensityPaceSecPerKm` against `mergedPolicy`, which already folds in the relative offset) but was never surfaced in the Absolute cell itself. Now: when `row.relativeTo !== "" && resolved != null`, the cell renders the same plain-text-plus-`.hra-anchor-tag` treatment the goal-time-derived case above already uses (`formatPaceSecPerKm(resolved)` + a new `"(resolved)"` tag) instead of the editable `<input>` — no input is even mounted in that branch, so AC2 ("no new editability") holds structurally, not just via the `disabled` attribute. A Relative row that DOESN'T resolve is untouched — falls through to the same disabled/empty `<input>` branch as before (AC3), since `absoluteDisabled`'s existing `row.relativeTo !== ""` condition already covers it.
  - **Testo del piano's own file upload — server-side .txt/.csv/.pdf extraction (HRA-328, Epic HRA-323).** A new upload control in the "Plan text" pipeline stage (distinct from the pre-existing "Upload .txt/.csv…" button in the Workout DSL stage, which still reads a file client-side straight into `dslSource` — unchanged) POSTs the file's base64 content to `POST /api/v1/source-files/extract` (HRA-327's endpoint; `api.sourceFiles.extract()`, new in `api/client.ts`) and places the returned `sourceText` into `originalText` for the user to review/correct — never `editor.dslSource`. `sourceUploading`/`sourceUploadError` are transient, un-stashed UI state (nothing to restore mid-upload); each of the backend's distinct extraction-error codes (`FILE_TOO_LARGE`/`TOO_MANY_PAGES`/`ENCRYPTED_PDF`/`INVALID_PDF`/`IMAGE_ONLY_PDF`/`EMPTY_EXTRACTION`/`INVALID_TEXT_ENCODING`) already carries its own human-readable `detail` message server-side (`domain/source-extraction.ts`), which `ApiError.message` surfaces verbatim — no new frontend error-code mapping needed; an unsupported extension is caught client-side before any request, showing the same `ErrorBanner`. Never calls `generatePromptFrom`/any AI path itself (Epic AC11). **Marks a previously-generated prompt/DSL stale on new upload (AC4)**: `promptStale`/`dslStale` flip `true` in the upload handler when `generatedPrompt`/`dslSource` are non-empty at upload time, surfaced via `conversionPromptStateLabel()`/`workoutDslStateLabel()` — the same "derivable status text" idiom HRA-238 established for those two functions (closing the gap that HRA-238's own comment flagged: "there is no stored baseline for generatedPrompt to diff against," for this one specific, upload-triggered case) — never by clearing/discarding the stale content itself. Each clears independently once addressed: `promptStale` on the next successful prompt generation (`generatePromptFrom`, either variant); `dslStale` on the next direct DSL edit (textarea `onChange` or the Workout DSL stage's own file upload). Both flags join the `Draft` stash/restore (`stashCurrentIfDirty`/`reopenDraft`) so switching rows never loses or fabricates the indicator.
  - **Accordion-based inline editing, Restore, pending-changes warning (HRA-141).** Same conversion HRA-140 already applied to `PlanTemplatesSection` — the old `mode: "list" | "plan"` full-screen swap is gone; every list row (plus a `"new"`-keyed draft row for an unsaved new instance, rendered above the list whenever `activeKey === "new"` or a `"new"` draft exists) is its own `AccordionCard`, keyed by `activeKey: number | "new" | null` instead of the page-level `mode` — `editingId` is now a derived const (`typeof activeKey === "number" ? activeKey : null`). Only one row's edits ever live in the pre-existing "live" editor state at a time, unchanged (`templateId`/`instName`/`raceName`/… and the whole HRA-136 dirty-bucket machinery). **The draft stash is larger than the template card's own `Draft`** — a `Draft` here captures BOTH every live field AND its own baseline (`baselineInstName`/`baselineStartDate`/`baselineAnchorRows`/… plus `persistedDsl`, the day-level baseline), not just the raw field values: reopening a stashed row has to reproduce the exact same `saveBucketDirty`/`regenerateBucketDirty` verdict it had when collapsed, and those two buckets are each computed by diffing live fields against baselines that don't exist anywhere else once the row is no longer active (unlike the template card's single `dslSource`, which could be diffed against the already-persisted `savedDslSource` on reopen without needing its own stashed baseline). `stashCurrentIfDirty(dirty)`/`onToggleRow(key, dirtyNow)`/`restoreDraft(draft)` mirror the template card's `stashCurrentIfDirty`/`onToggleRow`/`reopenDraft` shape exactly; `isDirty = saveBucketDirty || regenerateBucketDirty` (HRA-136's own union) is the one signal driving both the Restore confirm gate and the collapsed-row warning icon. **"Cancel" is renamed "Restore"** (reuses the shared `common.restore` key HRA-140 added), gated on a confirm modal only when `isDirty`; a clean row restores (resets + collapses) immediately. A drafted (dirty-but-collapsed) row shows the `AlertTriangle`-in-`.hra-text-warning` icon with a tooltip via a new `rowStatusHint()` (mirroring the template card's own); any other collapsed row shows the plain italic "Open to edit" hint. A stashed draft is dropped only by a successful Save, a successful Regenerate (both now clear the active row's `drafts` entry, matching HRA-136's own two "warning clears" triggers), or an explicit Restore — never by simply reopening the row. Save/Approve/Regenerate's HRA-136 gating formulas are untouched, just now also feeding the new `isDirty`/stash logic. **Delete uses the same overlay pattern HRA-140 shipped (round 2, per explicit instruction to carry it forward)**: a `Trash2` icon `<button>` positioned `absolute` + `zIndex: 1` over each row's own `AccordionCard`, a real DOM sibling inside a `position: relative` wrapper `<div>` — not nested in the header `<button>`, works whether the row is expanded or collapsed. This is new for the instance card specifically: before this Story, Delete only ever existed in the old `mode === "list"` screen (an inline confirm span, not an icon) and had no equivalent in `mode === "plan"` at all — approved here as a genuine addition, not a relocation of something the editor screen already had. One shared confirm modal (keyed off `deleteConfirmId` alone) renders at the `Card` level, same as the template card. Two new i18n keys, `manage.planInstances.openToEditHint`/`unsavedChanges`/`restoreConfirmBody` (English/Italian both added); the now-dead `manage.planInstances.editTitle` (the old `mode === "plan"` screen's own header text, with no reference left in the component) was removed from both locale files in the same edit.
