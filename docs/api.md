...
| `/api/v1/plan-instances/:id` | A resolved instance of a template — the instantiate response's own `Location` target — plus its `days` (all `plan_instance_days` rows, each with `segments` as JSON) and `approved_at` (HRA-113) |
...
| `/api/v1/plan-instances/:id` | Body `{days: [...]}` — full replacement of the instance's resolved days (structured JSON, not DSL text: an instance holds concrete `ResolvedDay` data). Rejects with 422 (listing which days) if any submitted day has `needs_review: true`. Never touches or re-instantiates the source template. Clears `approved_at` back to `NULL`, same gate-2 rule as the template PUT above. 404 if the instance doesn't exist (HRA-113) |
