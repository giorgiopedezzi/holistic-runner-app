...
describe("AccountPrivacySection", () => {
  it("renders the stored timezone as a selected canonical IANA option, not free text", async () => {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Account & privacy" }, { timeout: 5000 }));
  ...
  it("saving a profile change updates the sidebar's own display-name context (AC7), no decorative welcome copy needed", async () => {
    ...
    // Longer than the default 1000ms: expanding this section builds the
    // timezone combobox over the platform's full canonical IANA list (~400
    // options), which is comfortably fast alone but can exceed the default
    // under a full parallel suite run.
    const slow = { timeout: 5000 };
    expect(await screen.findByText("sidebarDisplayName:Giorgio", undefined, slow)).toBeInTheDocument();

    fireEvent.click(await screen.findByRole("button", { name: "Account & privacy" }, slow));
    const nameInput = await screen.findByDisplayValue("Giorgio", undefined, slow);
    fireEvent.change(nameInput, { target: { value: "New Name" } });
    fireEvent.click(screen.getByRole("button", { name: "Save profile" }));

    await waitFor(() => expect(screen.getByText("sidebarDisplayName:New Name")).toBeInTheDocument(), slow);
